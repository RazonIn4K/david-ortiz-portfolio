...
export function checkRateLimit(identifier, config = DEFAULT_CONFIG) {
  ...
  // Probabilistic cleanup: 1% chance to prevent memory leaks in serverless
  if (Math.random() < 0.01) {
...
// NOTE: setInterval-based cleanup removed to prevent memory leaks in serverless environments
// For serverless deployments, consider:
// 1. Using a distributed store with TTL (e.g., Upstash Redis)
// 2. Implementing probabilistic cleanup on each request (1% chance)
// 3. Using external cron jobs for cleanup

// Probabilistic cleanup: 1% chance on each rate limit check
// This is more suitable for serverless than setInterval
export function probabilisticCleanup() {
  if (Math.random() < 0.01) { // 1% chance
    return cleanupRateLimitStore();
  }
  return 0;
}
