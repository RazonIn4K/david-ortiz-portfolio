...
(function () {
  ...
  function buildTrackParams(element) {
    ...
    Object.keys(element.dataset).forEach((key) => {
      if (
        ...
        ["track", "trackCategory", "trackLabel"].includes(key)
      ) {
        return;
      }
  ...
  const featureDefinitions = [
      ...
      isAvailable: () =>
        Boolean(
          document.querySelector(CONTACT_FORM_SELECTOR) &&
            typeof window.EnhancedFormValidator !== "undefined",
        ),
      init: () =>
        new window.EnhancedFormValidator(CONTACT_FORM_SELECTOR, {
    ...
    },
    {
      ...
      isAvailable: () =>
        Boolean(
          window.CONFIG?.ENABLE_AI_CHAT &&
            typeof window.EnhancedAIChat !== "undefined" &&
            !window.enhancedChat &&
            !window.chatInstance,
        ),
      init: () => {
        ...
        return new window.EnhancedAIChat({
          enableAnalytics: true,
  ...
  const shortcutDefinitions = [
    {
      matches: (event) =>
        matchesShortcut(event, { key: "t", ctrlOrMeta: true, shift: true }),
      handler: () => {
    ...
    {
      matches: (event) =>
        matchesShortcut(event, { key: "k", ctrlOrMeta: true }),
      handler: () => {
        const contactInput = document.querySelector(
          `${CONTACT_FORM_SELECTOR} input[type="text"], ${CONTACT_FORM_SELECTOR} input[type="email"]`,
        );
        if (!contactInput) {
          return;
        }
  ...
  function setupKeyboardShortcuts() {
    document.addEventListener("keydown", (event) => {
      ...
      const shortcut = shortcutDefinitions.find((definition) =>
        definition.matches(event),
      );
      if (!shortcut) {
        return;
      }
  ...
  function matchesShortcut(
    event,
    { key, ctrlOrMeta = false, shift = false, alt = false },
  ) {
    const pressedKey = event.key?.toLowerCase();

    return (
      isCorrectKey(pressedKey, key) &&
  ...
  function initialize() {
    ...
    if (window.performance && window.performance.mark) {
      ...
      if (measure && window.CONFIG?.DEBUG) {
        console.debug(
          `Main initialization completed in ${Math.round(measure.duration)}ms`,
        );
      }
    }
    // Dispatch custom event for other scripts
    window.dispatchEvent(
      new CustomEvent("mainInitialized", {
